...
export const getters = {
  ...
  getNextEvents: (state, getters) => () => {
    ...
    const eventsOnSameDate = allEvents.filter((event) =>
      isSameDay(event.parsedDates, firstEventStartDate)
    )
  ...
  isCalendarLoading: (state) => (id) => {
    return (
      !state.calendars[id] ||
      (state.calendars[id] && state.calendars[id].isLoading)
    )
  },
}
...
export const actions = {
  async fetchEvents(
    { commit, state },
    { calendarId, startDate, endDate, limit }
  ) {
    if (!state.calendars[calendarId]) {
      commit('initCalendar', { calendarId, startDate, endDate })
    }

    await getEvents(calendarId, startDate, endDate, limit)
      .then((events) => {
...
export const mutations = {
  ...
  initCalendar(state, { calendarId, startDate, endDate }) {
    Vue.set(state.calendars, calendarId, {
